...
Widget::Widget(QWidget *parent): QWidget(parent), ui(new Ui::Widget)
{
    ...
    ui->groupBox->setTitle("Домашнее задание №1");

    connect(ui->ButtonStart, &QPushButton::clicked, this, &Widget::Simulation);//потом удали
...
{
    int AllNumberIsRight = 1;
    QList<QLineEdit*> ListOfLineEdit = {ui->EditWeight, ui->EditForce, ui->EditX0, ui->EditY0, ui->EditVx0,
                                    ui->EditVy0, ui->EditTime, ui->EditStepOfIntegration};
    QList<QLineEdit*>::iterator itLineEdit = ListOfLineEdit.begin();
    QRegularExpressionMatch match;
    QRegularExpression RXnumber("^-?\\d*\\.?\\d+$");
    while(itLineEdit != ListOfLineEdit.end())
    {
        match = RXnumber.match((**itLineEdit).text());
        if(!match.hasMatch())
        {
            AllNumberIsRight = 0;
            QMessageBox::warning(this, "Ошибка", "Введено не число: " + ((**itLineEdit).text()));
            break;
        }
        itLineEdit++;
    }
    if(ui->EditTime->text().toDouble() <= 0)
    {
        QMessageBox::warning(this, "Ошибка", "Введено отрицательное время: T = " + (ui->EditTime->text()));
        AllNumberIsRight = 0;
    }
    if(AllNumberIsRight == 1)
    {
        ...
        data.insert("L", data["m"]*(data["x"]*data["Vy"] - data["y"]*data["Vx"]));
        QList<qreal> DataResult({0, data["x"], data["y"], data["Vx"], data["Vy"], data["r"], data["V"], data["U"], data["K"], data["E"], data["L"]});
        data.insert("t", 0);
        data.insert("ax", 0);
        data.insert("ay", 0);
        ...
        while(data["t"] < data["T"])
        {
            ...
            data["V"] = pow(pow(data["Vx"], 2) + pow(data["Vy"], 2), 0.5);
            ...
            data["L"] = data["m"]*(data["x"]*data["Vy"] - data["y"]*data["Vx"]);
            ...
            items.append(new QStandardItem(QString::number(data["L"], 'g', 3)));
            TableModel->appendRow(items);
            //Sleep(500);
            items.clear();
        }
